...
tsproc.prototype.getAvgPerDay = function(){
	...
	if (this.isHomogeneous()){
		...
		if (this.getNbTS() === 1){
			...
			// var diff, cumul_diff = 0;
			// var n = this.getTSSize();

			// //day length in milliseconds
			// //var day_length = 86400000*60;
			...
			// 	diff = date.diff(old_date, 'year', true);
			// 	cumul_diff += diff;

			// 	console.log(date.toISOString().slice(0,10) + ' diff = ' + diff + ' , cumul: ' + cumul_diff);

			// 	if (cumul_diff === 1){

			// 		cumul_diff = 0;
			// 		console.log('per day: ' + per_day_count);
			// 		per_day_count = 0;
			// 	}
