	...
	if (this.isHomogeneous()){
		...
		if (this.getNbTS() === 1){
			...
			for (var i=1; i<n; i++){
				...
				console.log('date: ' + date.get(format) + ', old: ' + old_date.get(format));
				...
				if (diff1 === 0){
					...
					//if the same days of diffrenet monthes
					if (diff2 > 1){
						...
						i--;
						old_date = this.getTimestamp(0,i);
						chunks_count++;
					}
				}
				...
				console.log(date.toISOString().slice(0,10) + ' diff1 = ' + diff1 + ', diff2 = ' + diff2);
