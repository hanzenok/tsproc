	...
	if (this.isHomogeneous()){
		...
		if (this.getNbTS() === 1){
			...
			for (var i=1; i<n; i++){
				...
				console.log('date: ' + date.toISOString() + ', old: ' + old_date.toISOString());
				diff1 = date.get(format) - old_date.get(format);
				diff2 = Math.ceil(date.diff(old_date, 'days', true));
				...
				if (diff1 === 0){
					...
					//console.log('zero: ' + date.toISOString().slice(0,10));
				
					//if the same days of diffrenet monthes or years
					if (diff2 > 1){
						...
						while (diff1 === 0 && i < n){
							...
							console.log('inside: ' + date.toISOString() + ' diff1 = ' + diff1 + ', diff2 = ' + diff2);
						}
						...
						console.log('i=' + i + ', n=' + n);
						console.log('date: ' + date.toISOString() + ', old: ' + old_date.toISOString());
						//continue; //to prvent the rewriting of old_date
					}
				}
			...
			//count the last one
			sum += 1/diff2;
			if (diff2 > 1) chunks_count++;

			console.log('sum: ' + sum);
			console.log('days: ' + chunks_count);
			console.log('mean: ' + sum/chunks_count);

			var min_date = getMinDate.call(this);
			var max_date = getMaxDate.call(this);

			console.log('min: ' + min_date.toISOString() + ', max: ' + max_date.toISOString());
			console.log('nb_days: ' + max_date.diff(min_date, 'days', true) + ', per_day: ' + (n/Math.ceil(max_date.diff(min_date, 'days', true))));
