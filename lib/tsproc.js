...
tsproc.prototype.process = function(callback){
	...
	if (this.isValid()){
		...
		this.checkSimilarity(null);
		//console.log(getCorrelationCoef.call(this, [1, 0, 3, 0, 5], [1, 0, 0, 0, 5]));
 ...
 * @param {int} ts_index The index of timeseries
 * @param {int} doc_index The index of the document in the timeseries
 * @return {string} Requested document
 */
...
tsproc.prototype.checkSimilarity = function(callback){
	...
	if (this.getNbTS() === 1 && this.getFields(0).length > 1){
		...
		// values = extractValues.call(this);
		// console.log('values: ');
		// console.log(values);
		// console.log('coef: ' + getCorrelationCoef.call(this, values[0], values[1]));
		...
		for (var i=0; i<ts_size-3; i++){
			...
			for (var k=ts_size-1; k>i; k--){
				...
				if (coef > 0.6){
					...
					markCorrelated.call(this, [i, k], true);
			...
			//mark all the rest of
			//doucments as not correlated
			markCorrelated.call(this);
		}
...
//adds a field correlated to each document of timeseries
//with indication wheather this document is a part of a local correlation
//between timeseries
//if correlation state is already defined
//it does not overrides it
function markCorrelated(borders, isCorrelated){
	...
	if (this.getNbTS() === 1 && this.getFields(0).length > 1){
		...
		//correlation state
		var correlated = (isCorrelated) ? isCorrelated : false;
		...
		for (var i=start_index; i<=end_index; i++){
			...
			//if state is not yet defined
			if (!doc.correlated)
				doc.correlated = correlated;
		}
	}
}
