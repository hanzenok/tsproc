 ...
 * - if the are multiple timeseries (and after the previous step they are all homogeneous), they would be merged into one timeseries document by document
 * - the output timeseries's timestamp field would be called 'time' and it would be an ISO string
 * - if the skip value is defined in the JSON config, the timeseries would be undersampled to decrease it's size
 * - finally, if the quantification is demanded (in the JSON config), the values of the timeseries (timestamps and normal) would be quantified
 * 
 * @method process
 ...
 *
 * <i>Note</i>: for now, it returns only the timeseries config
 * of the <code>tsproc</code> (not the options like transform type,
 ...
 * (it's name is specified by <code>TIMESTAMP_FIELD</code> proprety).
 *
 * @method merge
 * @param {function} callback A callback function
 *
 ...
 * Uses a <code>tsinterp</code> module.
 *
 * @method interpolate
 ...
 * 						],
 *
 * 					"timestamp":{"name":"year","format":"YYYY-MM", "quantum": "year"}	//new field
 * 					},
 *
 * 					{
 * 					"fields":
 ...
 * <code>TIMESTAMP_FIELD</code> proprety.
 *
 * @method mergeConfigs
 * @private
 */
