...
tsproc.prototype.process = function(callback){
	...
	if (this.isValid()){
		...
		console.log(getCorrelation.call(this, [1, 2, 3, 4, 5, 6], [2, 2, 3, 4, 5, 60]));
	}
	else{
...
function getCorrelation(array1, array2){
	...
	if (array1 && array1.length && array2 && array2.length){
		...
		coef  = (n*sum_xy - sum_x*sum_y) / (Math.sqrt( (n*sum_x2 - sum_x*sum_x) * (n*sum_y2 - sum_y*sum_y) ));
	}
