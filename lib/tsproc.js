...
function tsproc(timeseries, config, err_callback){
	...
	if (timeseries && config){
		...
		if (config){
			...
			this._count_negative = (config.correlation && typeof config.correlation.count_negative !== 'undefined') ? config.correlation.count_negative : false;
			this._max_coef = (config.correlation && typeof config.correlation.max_coef !== 'undefined') ? config.correlation.max_coef : true;

			console.log('count_negative: ' + this._count_negative + ', max_coef: ' + this._max_coef);
	...
	if (this.getNbTS() === 1 && this.getFields(0).length > 1){
		...
		for (var i=0; i<ts_size-3; i++){
			...
			for (var k=ts_size-1; k>i; k--){
				...
				coef = (this._count_negative) ? Math.abs(coef) : coef;
				if (coef > 0.6){
					...
					markCorrelation.call(this, [i, k], max);
...
function markCorrelation(borders, corr_value){
	...
	if (this.getNbTS() === 1 && this.getFields(0).length > 1){
		...
		//correlation value
		var correlation = (corr_value) ? +corr_value : 0;
