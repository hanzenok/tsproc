...
tsproc.prototype.intersect = function(callback){
	...
	if (nb_ts > 1){
		...
		if (this.isNominal()){
			...
			if (callback)
				callback(new Error('Intersection cannot be used for nominal timeseries'), this.getTS());
...
tsproc.prototype.checkSimilarity = function(callback){
	...
	if (this._correlation_activated && this.getNbTS() === 1 && this.getFields().length === 2 && !this.isNominal()){
 ...
 * Also, the method should be used only by timeseries with numeric fields (non-nominal).
 * 
 * @method quantize
 * @param {function} callback A callback function
 *
...
tsproc.prototype.quantize = function(callback){
	...
	if (this.getNbTS() === 1 && this.isISO()){
		...
		//only for non-nominal timeseries
		if (this.isNominal()){

			if (callback)
				callback(new Error('Quantification cannot be applied to the nominal timeseries'), this.getTS());
			return;
		}
