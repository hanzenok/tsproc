 ...
 * with multiple timeseries and offers different public methods
 * to process and explore them.
 *
 * @class tsproc
 * @constructor
 * @description baasdf fasdf asdf
 * @for tsproc
...
/**
 * A main public method that processes a set of timeseries. It returns (via callback) a single timeseries with timestamp 'time' with
 * ISO format.
 *
 ...
 * - if there are multiple timeseries, and they are not homogeneous, all the timeseries would be interpolated or intersected (cf intersect() function)
...
/**
 * Public method that returns (directly or via callback) 
 * one timeseries if there is one,
...
/**
 * Public method that takes a timestamp and returns (via callback) 
...
/**
 * Public method that returns an array of fields present 
 * in the specified timeseries (ie in each of it's document).
 *
 * @method getFields
 * @param {int} ts_index The index of timeseries
 ...
 * @return {boolean} Wheather a set of timeseries are homogeneous
 *
 * @example
 *     var tsproc = require('tsproc');
 *
 ...
 * - skip: choose one document (first) from skipsize documents
 ...
 * @param {int} skipsize An index of ReduceEnum with reduction methods from ReduceFunc
...
/**
 * Public mehtod that could be applied to two or more timeseries
 ...
 * timestamps id different timeseries (cf example).
 *
 * Timestamps should be the ISO strings.
 * @method intersect
 * @param {function} callback A callback function
 ...
 *
 * Timeseries should be merged (ie the function works with 
 * only a single timeseries) and have at least two non-timestamp fields.
 * 
...
/**
 * Private method that cuts in a single timeseries all the documents
 ...
 * (specified by the 'borders') a field 'correlation' with value
 * 'corr_value' (cf example).
 *
