...
tsproc.prototype.getAvgPerDay = function(){
	...
	if (this.isHomogeneous()){
		...
		if (this.getNbTS() === 1){
			...
			var sum = 0;
			var days_count = 0;
			var per_day_count = 1; //i=0 is counted
			...
			for (var i=1; i<n; i++){
				...
				diff = date.get(format) - old_date.get(format);
				...
				if (diff === 0){
					...
					sum += per_day_count; //accumulate the sum
					per_day_count = 1; //reinit
					days_count++; //count days
				}

				console.log(date.toISOString().slice(0,10) + ' difff = ' + diff + ', per_day: ' + per_day_count);
